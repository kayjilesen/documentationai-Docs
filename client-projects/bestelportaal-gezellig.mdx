...
| **Lokaal** | `gezellig-bestellen.local` (of project-`.local`) | Detectie via `is_live_domain()` / `.local` |

<Callout kind="info" title="Omgeving detectie">
  Lokale detectie gebeurt via het hostdomein (o.a. `.local`), niet via poort `:8888`. Zie [Lokale Ontwikkelomgeving](/devops/local-dev).
</Callout>
...
<Callout kind="info" title="SMTP">
  Transactionele mail via [FluentSMTP](/wordpress/plugins/fluent-smtp) (Maileroo + SES-backup). Documenteer provider/from in dit projectdossier zodra bekend; geen credentials hier.
</Callout>

| Item | Waarde |
|------|--------|
| **SMTP Plugin** | FluentSMTP (verwacht) |
| **Provider** | *Projectspecifiek — zie FluentSMTP-standaard* |
| **From** | *Nog te documenteren* |
...
- **WordPress admin (productie)**: bij voorkeur `/beheer` als Defender Mask Login actief is; anders `/wp-admin` (controleer live)
