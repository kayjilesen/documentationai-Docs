...
xCloud kan transactionele e-mails versturen via **FluentSMTP**. Standaard: **Maileroo** (`noreply@klant`) als primair + **Amazon SES** (`@kayjilesen.dev`, Stockholm) als backup — credentials in `wp-config.php`, Pushover voor alerts. Zie [FluentSMTP](/wordpress/plugins/fluent-smtp).

<Callout kind="warning" title="E-mail testen">
  Configureer altijd beide connections en stuur testmails. Standaard server-mail is onbetrouwbaar. Zet SMTP-/AWS-keys nooit in Git.
</Callout>
...
<Columns cols={3}>
  ...
  <Card title="Caching" icon="zap" href="/hosting/caching">
    FastCGI Nginx + Redis.
  </Card>
