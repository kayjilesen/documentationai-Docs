...
<Callout kind="info" title="Waarom dit kennen?">
  Bij migraties, broken media, ACF-problemen of “waar staat dit eigenlijk?” denk je eerst: staat het in de **database**, in **bestanden** (`wp-content`), of in **beide**?
</Callout>
...
<Callout kind="tip" title="JSON + Sync">
  Definitie leeft in Git (`acf-json/`). Op een omgeving: **ACF → Sync**. Zie [ACF Patronen](/wordpress/acf-patterns).
</Callout>
...
1. **Bestand** onder `wp-content/uploads/…` — zie [wp-content & uploads](/wordpress/wp-content)
...
- [wp-content & uploads](/wordpress/wp-content) — wat op schijf staat vs. in de DB
