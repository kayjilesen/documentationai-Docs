...
<Callout kind="info" title="Eerst de fundamenten?">
  Nog nieuw met WordPress zelf? Lees in tab **Kennis**: [Database](/wordpress/database), [Bestanden](/wordpress/bestanden) en [Configuratie](/wordpress/configuratie) voordat je in de theme-structuur duikt.
</Callout>
...
<Steps>
  ...
  <Step title="Voeg blocks toe" icon="layers">
    Maak pagebuilder blocks aan via [Pagebuilder](/acf/pagebuilder). De boilerplate bevat al `hero` en `text-image` als voorbeeld.
  </Step>
